...
func testUserToManyCreatedUserPersons(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	b.CreatedUserID = a.UserID
	c.CreatedUserID = a.UserID
	...
	check, err := a.CreatedUserPersons().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadCreatedUserPersons(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreatedUserPersons); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.CreatedUserPersons = nil
	if err = a.L.LoadCreatedUserPersons(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreatedUserPersons); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyUpdatedUserPersons(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	b.UpdatedUserID = a.UserID
	c.UpdatedUserID = a.UserID
	...
	check, err := a.UpdatedUserPersons().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if v.UpdatedUserID == b.UpdatedUserID {
			bFound = true
		}
		if v.UpdatedUserID == c.UpdatedUserID {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUpdatedUserPersons(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UpdatedUserPersons); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UpdatedUserPersons = nil
	if err = a.L.LoadUpdatedUserPersons(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UpdatedUserPersons); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpCreatedUserPersons(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddCreatedUserPersons(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.UserID != first.CreatedUserID {
			t.Error("foreign key was wrong value", a.UserID, first.CreatedUserID)
		}
		if a.UserID != second.CreatedUserID {
			t.Error("foreign key was wrong value", a.UserID, second.CreatedUserID)
		}
		...
		if a.R.CreatedUserPersons[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.CreatedUserPersons[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.CreatedUserPersons().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpUpdatedUserPersons(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddUpdatedUserPersons(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.UserID != first.UpdatedUserID {
			t.Error("foreign key was wrong value", a.UserID, first.UpdatedUserID)
		}
		if a.UserID != second.UpdatedUserID {
			t.Error("foreign key was wrong value", a.UserID, second.UpdatedUserID)
		}

		if first.R.UpdatedUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.UpdatedUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.UpdatedUserPersons[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UpdatedUserPersons[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UpdatedUserPersons().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`UserID`: `bigint`, `FirstName`: `varchar`, `LastName`: `varchar`}
	_           = bytes.MinRead
)
