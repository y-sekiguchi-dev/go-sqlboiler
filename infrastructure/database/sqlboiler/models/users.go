...
var UserRels = struct {
	CreatedUserPersons string
	UpdatedUserPersons string
}{
	CreatedUserPersons: "CreatedUserPersons",
	UpdatedUserPersons: "UpdatedUserPersons",
}

// userR is where relationships are stored.
type userR struct {
	CreatedUserPersons PersonSlice
	UpdatedUserPersons PersonSlice
}
...
var (
	userAllColumns            = []string{"user_id", "first_name", "last_name"}
	userColumnsWithoutDefault = []string{"first_name", "last_name"}
...
// CreatedUserPersons retrieves all the person's Persons with an executor via created_user_id column.
func (o *User) CreatedUserPersons(mods ...qm.QueryMod) personQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`persons`.`created_user_id`=?", o.UserID),
	)
...
// UpdatedUserPersons retrieves all the person's Persons with an executor via updated_user_id column.
func (o *User) UpdatedUserPersons(mods ...qm.QueryMod) personQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`persons`.`updated_user_id`=?", o.UserID),
	)
...
// LoadCreatedUserPersons allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCreatedUserPersons(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`persons`), qm.WhereIn(`persons.created_user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.CreatedUserPersons = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UserID == foreign.CreatedUserID {
				local.R.CreatedUserPersons = append(local.R.CreatedUserPersons, foreign)
				if foreign.R == nil {
					foreign.R = &personR{}
				}
...
// LoadUpdatedUserPersons allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUpdatedUserPersons(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`persons`), qm.WhereIn(`persons.updated_user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.UpdatedUserPersons = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &personR{}
			}
			foreign.R.UpdatedUser = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UserID == foreign.UpdatedUserID {
				local.R.UpdatedUserPersons = append(local.R.UpdatedUserPersons, foreign)
				if foreign.R == nil {
					foreign.R = &personR{}
				}
				foreign.R.UpdatedUser = local
				break
			}
		}
	}
...
// AddCreatedUserPersonsG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreatedUserPersons.
// Sets related.R.CreatedUser appropriately.
// Uses the global database handle.
func (o *User) AddCreatedUserPersonsG(ctx context.Context, insert bool, related ...*Person) error {
	return o.AddCreatedUserPersons(ctx, boil.GetContextDB(), insert, related...)
}

// AddCreatedUserPersons adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreatedUserPersons.
// Sets related.R.CreatedUser appropriately.
func (o *User) AddCreatedUserPersons(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Person) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.CreatedUserID = o.UserID
			...
			values := []interface{}{o.UserID, rel.PersonID}
	...
	if o.R == nil {
		o.R = &userR{
			CreatedUserPersons: related,
		}
	} else {
		o.R.CreatedUserPersons = append(o.R.CreatedUserPersons, related...)
	}
...
// AddUpdatedUserPersonsG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UpdatedUserPersons.
// Sets related.R.UpdatedUser appropriately.
// Uses the global database handle.
func (o *User) AddUpdatedUserPersonsG(ctx context.Context, insert bool, related ...*Person) error {
	return o.AddUpdatedUserPersons(ctx, boil.GetContextDB(), insert, related...)
}

// AddUpdatedUserPersons adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UpdatedUserPersons.
// Sets related.R.UpdatedUser appropriately.
func (o *User) AddUpdatedUserPersons(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Person) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UpdatedUserID = o.UserID
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
	...
	if o.R == nil {
		o.R = &userR{
			UpdatedUserPersons: related,
		}
	} else {
		o.R.UpdatedUserPersons = append(o.R.UpdatedUserPersons, related...)
	}
...
func FindUserG(ctx context.Context, userID int64, selectCols ...string) (*User, error) {
...
func UserExistsG(ctx context.Context, userID int64) (bool, error) {
